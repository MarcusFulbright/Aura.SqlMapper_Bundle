...
class EntityExtractor
{
    ...
    {
        $output = array();
        if ($persist_order = $mapper->getPersistOrder()) {
     ...
     *
     */
    protected function iterateOverProperties(
        $object,
    ...
    protected function getPlaceholder($relation_name, AggregateMapperInterface $mapper, array $output) {
        ...
        $index = $this->getIndexForPlaceholder($join_address, $relation['owner'], $output);
     ...
     *
     */
    protected function getIndexForPlaceholder($join_address, $row_has_been_created, array $output)
    {
        $index = 0;
