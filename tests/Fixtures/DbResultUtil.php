...
trait DbResultUtil
{
    ...
    public function formatRecordToObject(array $data, $entity = false)
    {
        if (isset($data['__root'])) {
            $output = $data['__root'][0];
        }
        if (isset($data['floor'])) {
            $output->floor = $entity === false ? $data['floor'][0] : $data['floor'][0]->id;
        }
        if (isset($data['building'])){
            $output->building = $entity === false ? $data['building'][0] : $data['building'][0]->id;
        }
        if (isset($data['building.type']) && $entity === false) {
            $output->building->type = $data['building.type'][0];
        }
        if (isset($data['task']) && $entity === false) {
            $output->task = $data['task'];
            foreach ($output->task as $task) {
                unset($task->userid);
            }
        }
        if (isset($data['task.type']) && $entity === false) {
